...
// src: 
// https://github.com/heremaps/flexible-polyline/tree/master/dotnet
    public class PolylineEncoderDecoder
    {
        ...
        public static string Encode(List<LatLngZ> coordinates, int precision, ThirdDimension thirdDimension, int thirdDimPrecision)
        {
            ...
            var enc = new Encoder(precision, thirdDimension, thirdDimPrecision);
        ...
        public static T[] Decode<T>(string encoded, Func<LatLngZ, T> createFunc)
        {
            return Decode(encoded).Select(createFunc).ToArray();
        }
        ...
        public static IEnumerable<LatLngZ> Decode(string encoded)
        {
        ...
        public class Converter
        {
            ...
            public void EncodeValue(double value, StringBuilder result)
            {
				 ...
				 * round(-2.5) --> -3
				 */
                var scaledValue = (long)Math.Round(Math.Abs(value * _multiplier), MidpointRounding.AwayFromZero) * Math.Sign(value);
                var delta = scaledValue - _lastValue;
...
public class LatLngZ
{
    ...
    public override bool Equals(object obj)
    {
        ...
        if (obj is not LatLngZ latLngZ) return false;
       
        return latLngZ.Lat == Lat && latLngZ.Lng == Lng && latLngZ.Z == Z;
    }
